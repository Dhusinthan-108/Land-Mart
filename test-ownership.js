...
async function testOwnership() {
    try {
        ...
        const userResponse = await axios.post('http://localhost:5501/api/users', {
            name: 'Test Owner',
        ...
        const propertyResponse = await axios.post('http://localhost:5501/api/properties', {
        ...
        const user2Response = await axios.post('http://localhost:5501/api/users', {
            name: 'Another User',
        ...
        const property2Response = await axios.post('http://localhost:5501/api/properties', {
        ...
        const userProperties = await axios.get(`http://localhost:5501/api/properties/user/${userId}`);
        console.log(`User ${userId} has ${userProperties.data.length} properties:`);
        userProperties.data.forEach(prop => {
            console.log(`  - ${prop.title}`);
        });
        
        const user2Properties = await axios.get(`http://localhost:5501/api/properties/user/${user2Id}`);
        ...
        const allProperties = await axios.get('http://localhost:5501/api/properties');
